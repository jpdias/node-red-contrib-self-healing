...
describe('readings-watcher node', function () {
	...
	beforeEach(function (done) {
		testflow =
		[
			{ "id": "watcher", "type": "readings-watcher", "name": "readings-watcher", "wires": [["ok"], ["error"]] },
			{ "id": "ok", "type": "helper" },
	...
	it('should be loaded', function (done) {
		let flow = [ {id: "watcher", type: "readings-watcher", name: "readings-watcher" }];

		helper.load(readingswatcher, flow, function () {
	...
	it('should allow consecutive readings when using \'minchange\' and their values are sufficiently different', function (done) {
		...
		let minchange = 0.1;
		minchange.should.be.lessThan(Math.abs(messageSequence[0]-messageSequence[1])/messageSequence[0]);
		...
		helper.load(readingswatcher, testflow, function () {
			...
			ok.on("input", function (msg) {
				try {
					...
					if(receivedMsgs == messageSequence.length)
						done();
				} catch (err) {
				  done(err);
				}
	...
	it('should allow consecutive readings when using \'maxchange\' and their values are sufficiently similar', function (done) {
		...
		let maxchange = 0.5;
		maxchange.should.be.greaterThan(Math.abs(messageSequence[0]-messageSequence[1])/messageSequence[0]);
	...
	it('should trigger an error when using \'minchange\' and consecutive readings are too similar', function (done) {
		...
		let minchange = 0.1;
		minchange.should.be.greaterThanOrEqual(Math.abs(messageSequence[0]-messageSequence[1])/messageSequence[0]);
		testflow[0].strategyMask = 1;
		testflow[0].minchange = minchange;
	...
	it('should trigger an error when using \'maxchange\' and consecutive readings are too different', function (done) {
		...
		let maxchange = 0.65;
		maxchange.should.be.lessThanOrEqual(Math.abs(messageSequence[0]-messageSequence[1])/messageSequence[0]);
		testflow[0].strategyMask = 2;
		testflow[0].maxchange = maxchange;
